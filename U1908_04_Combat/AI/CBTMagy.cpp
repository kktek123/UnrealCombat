...
UCBTMagy::UCBTMagy()
{
	StateBehaviorRange = 700;
	FleeBehaviorRange = 300;
}

void UCBTMagy::TickNode(UBehaviorTreeComponent & OwnerComp, uint8 * NodeMemory, float DeltaSeconds)
{
	...
	Update();
	ControlledCharacter->GetStateManager()->OnStateChanged.AddDynamic(this, &UCBTMagy::OnStateChanged);

}

void UCBTMagy::OnStateChanged(E_State PrevState, E_State NewState)
{
	if (PrevState == E_State::Disabled || NewState == E_State::Disabled
		||  NewState == E_State::Attacking 
		 || NewState == E_State::Damage)
	{
		Update();
	}
}

void UCBTMagy::Update()
{
	UpdateBehavior();
}

void UCBTMagy::UpdateBehavior()
{
	...
	// = Cast<ACEnemy>(MyBlackboardComponent->GetValueAsObject("Patient"));
	...
	//PatientHealth = Patient->GetExtendedHealth()->GetCurrentValue()
	//	/ Patient->GetExtendedHealth()->GetMaxValue()
	//	* 100.0;
	...
	if (ControlledCharacter->bGetDamaged)
	{
		//CLog::Print(TEXT("Hit"));
		//SetBehavior(EC_AIBehavior::Hit);
		ControlledCharacter->GetWorldTimerManager().SetTimer(CountdownTimerHandle, this, &UCBTMagy::Flee, 1.0f, false);
		ControlledCharacter->bGetDamaged = false;
		return;
	}
			...
			else
			{
				...
				else
				{
					TicksNearTarget = 0;
					{
						...
						//else if (PatientHealth <= 70.0f)
						//{
						//	SetBehavior(EC_AIBehavior::);
						//}
						//		CLog::Print(TEXT("StrafeAround"));
					}
					//if (DistanceToTarget >= 900)
...
void UCBTMagy::UpdateActivities()
{
	RetNull(AOwnerController);
	RetNull(ControlledCharacter);
...
void UCBTMagy::Flee()
{
	if ((EC_AIBehavior)MyBlackboardComponent->GetValueAsEnum("CBehavior") == EC_AIBehavior::Hit)return;
	//CLog::Print(FName("Flee"));
	SetBehavior(EC_AIBehavior::Flee);
}

void UCBTMagy::GetDamage()
{
	if (ControlledCharacter->GetStateManager()->GetState() == E_State::Damage)
	{
		//CLog::Print(TEXT("Hit"));
		//SetBehavior(EC_AIBehavior::Hit);
		ControlledCharacter->GetWorldTimerManager().SetTimer(CountdownTimerHandle, this, &UCBTMagy::Flee, 1.0f, false);
...
void UCBTMagy::SetBehavior(EC_AIBehavior Behavior)
{
